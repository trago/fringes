...
dc = np.ones_like(phase)*0.1 # + functions.gaussian(100, shape) * 5.0
contrast = 1 # + functions.gaussian(100, shape)*10 + 2
noise = .0

image_list = [dc + contrast * np.cos(phase + d) + np.random.randn(*shape) * noise for d in delta]

#step, dc_, pp = demodulate_2frames(image_list, patch_size=48)
...
image_list = [image[:-2, :-2] - dc_ for image in image_list]
pp = phase_2steps(image_list[0], image_list[1], step)
